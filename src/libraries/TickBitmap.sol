...
library TickBitmap {
    ...
    /// @param bitmap the tick bitmap compute the next initialized tick
    /// @param bitPos The bit position of `bitmap` for the tick in question
    /// @param compressed The compressed tick value, tick / tickSpacing rounded towards negative infinity
    /// @param tickSpacing The spacing between usable ticks
    ...
    function nextInitializedTickWithinOneWord(
        ...
        unchecked {
            ...
            (wordPos, bitPos) = lte ? position(compressed) : position(++compressed);
        }
        (next, initialized) = nextInitializedTickWithinOneWord(self[wordPos], bitPos, compressed, tickSpacing, lte);
    }
}
