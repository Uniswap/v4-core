...
library Pool {
    ...
    struct Slot0 {
        ...
        // upper 16 bits are for 1->0, and the lower 16 are for 0->1
    ...
    function initialize(State storage self, uint160 sqrtPriceX96, uint32 protocolFee, uint24 swapFee)
        internal
        returns (int24 tick)
    {
    ...
    function setProtocolFee(State storage self, uint32 protocolFee) internal {
        ...
        returns (BalanceDelta result, uint256 feeForProtocol, uint24 swapFee, SwapState memory state)
    {
        ...
        SwapCache memory cache = SwapCache({
            ...
            protocolFee: params.zeroForOne ? uint16(slot0Start.protocolFee % 65536) : uint16(slot0Start.protocolFee >> 16)
        });
