...
library Pool {
    ...
    /// @return deltas the deltas of the token balances of the pool, from the liquidity change
    /// @return feesAccrued the fees generated by the liquidity range
    function modifyLiquidity(State storage self, ModifyLiquidityParams memory params)
        internal
        returns (BalanceDeltas deltas, BalanceDeltas feesAccrued)
    {
...

        {
...

            {
                ...
                feesAccrued = toBalanceDeltas(feesOwed0.toInt128(), feesOwed1.toInt128());
            }
    ...
    function swap(State storage self, SwapParams memory params)
        internal
        returns (BalanceDeltas result, uint256 feeForProtocol, uint24 swapFee, SwapState memory state)
    {
        ...
        if (params.amountSpecified == 0) return (BalanceDeltasLibrary.ZERO_DELTAS, 0, swapFee, state);
    ...
    function donate(State storage state, uint256 amount0, uint256 amount1) internal returns (BalanceDeltas deltas) {
        ...
        unchecked {
            ...
            deltas = toBalanceDeltas(-(amount0.toInt128()), -(amount1.toInt128()));
            if (amount0 > 0) {
