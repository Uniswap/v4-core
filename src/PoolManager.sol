...
contract PoolManager is IPoolManager, Fees, NoDelegateCall, ERC6909Claims {
    ...
    function getSlot0(PoolId id)
        ...
        view
        override
        returns (uint160 sqrtPriceX96, int24 tick, uint32 protocolFee)
    {
        ...
        returns (int24 tick)
    {
        ...
        PoolId id = key.toId();
        (, uint32 protocolFee) = _fetchProtocolFee(key);
    ...
    function setProtocolFee(PoolKey memory key) external {
        (bool success, uint32 newProtocolFee) = _fetchProtocolFee(key);
