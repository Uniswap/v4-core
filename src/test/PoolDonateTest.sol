...
contract PoolDonateTest is ILockCallback {
        ...
        returns (BalanceDelta delta)
    {
        bytes memory data = abi.encode(
            DonateData(DonateType.Single, abi.encode(CallbackData(msg.sender, key, amount0, amount1, hookData)))
        );
    ...
    function lockAcquired(bytes calldata rawData) external returns (bytes memory) {
        ...
        if (donateData.donateType == DonateType.Single) {
            ...
            key = data.key;
            sender = data.sender;
            delta = manager.donate(data.key, data.amount0, data.amount1, data.hookData);
