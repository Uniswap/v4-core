...
contract ProtocolFeeControllerTest is IProtocolFeeController {
    ...
    mapping(PoolId => uint32) public swapFeeForPool;

    function protocolFeeForPool(PoolKey memory key) external view returns (uint32) {
        return swapFeeForPool[key.toId()];
    }

    // for tests to set pool protocol fees
    function setSwapFeeForPool(PoolId id, uint32 fee) external {
...
contract RevertingProtocolFeeControllerTest is IProtocolFeeController {
    function protocolFeeForPool(PoolKey memory /* key */ ) external pure returns (uint32) {
        revert();
    }
}
