...
contract PoolSwapTest is PoolTestBase {
    ...
    function unlockCallback(bytes calldata rawData) external returns (bytes memory) {
        ...
        require(deltaBefore0 == 0);
        require(deltaBefore1 == 0);
        ...
        require(reserveBefore0 == reserveAfter0);
        require(reserveBefore1 == reserveAfter1);

        if (data.params.zeroForOne) {
            if (data.params.amountSpecified < 0) {
                // exact input, 0 for 1
                require(deltaAfter0 == data.params.amountSpecified);
                require(deltaAfter1 > 0);
            } else {
                // exact output, 0 for 1
                require(deltaAfter0 < 0);
                require(deltaAfter1 == data.params.amountSpecified);
            }
        } else {
            if (data.params.amountSpecified < 0) {
                // exact input, 1 for 0
