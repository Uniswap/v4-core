...
library Hooks {
    ...
    function validateHookAddress(IHooks self, Calls memory calls) internal pure {
        if (
            calls.beforeInitialize != shouldCall(self, CallPoint.BeforeInitialize) ||
            calls.afterInitialize != shouldCall(self, CallPoint.AfterInitialize) ||
            calls.beforeSwap != shouldCall(self, CallPoint.BeforeSwap) ||
            calls.afterSwap != shouldCall(self, CallPoint.AfterSwap) ||
            calls.beforeModifyPosition != shouldCall(self, CallPoint.BeforeModifyPosition) ||
            calls.afterModifyPosition != shouldCall(self, CallPoint.AfterModifyPosition) ||
            calls.beforeDonate != shouldCall(self, CallPoint.BeforeDonate) ||
            calls.afterDonate != shouldCall(self, CallPoint.AfterDonate)
        ) {
            revert HookAddressNotValid(address(self));
        }
    }

    function shouldCall(IHooks self, CallPoint callPoint) internal pure returns (bool) {
        return uint256(uint160(address(self))) & (1 << (159 - uint256(callPoint))) != 0;
    }
    ...
    //     return shouldCall(self, CallPoint.BeforeSwap);
    // }
