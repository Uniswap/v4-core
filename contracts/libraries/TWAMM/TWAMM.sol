...
library TWAMM {
    ...
    function advanceTimestampForSinglePoolSell(
        ...
        // TODO
        uint256 amountSelling = (sellRateCurrent * params.secondsElapsed) >> FixedPoint96.RESOLUTION;
    ...
    function advanceToNewTimestamp(
        ...
        // we may cross an initialized tick in the swap, if we do.. then we have to advance the time to the tick crossing
        // this naming is confusing, if we are currently crossing a tick this will be false which doesnt make sense to me, what are the cases?
        // should be if (aboutToCrossInitTick)
        if (crossingInitializedTick) {
            // TODO need to adjust calculations for when only one pool is selling
            return
            ...
            // currently CROSSING tick OR will never cross init tick
            console.log('not crossing init tick');
            if (params.nextTimestamp % self.expirationInterval == 0) {
                console.log('exp on interval');
                // if the next timestamp is on an interval advance to the interval
                ...
                console.log('exp not on interval');
    ...
    function calculateEarnings() internal pure {}
    ...
    // gets the next initialized tick or the closest uninit tick to the target tick
    // if there are no initialized ticks before the target tick
