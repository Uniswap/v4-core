...
contract MockHooks is IHooks, IHookFeeManager {
    ...
    function beforeInitialize(address, PoolKey memory, UQ64x96) external view override returns (bytes4) {
        bytes4 selector = MockHooks.beforeInitialize.selector;
        return returnValues[selector] == bytes4(0) ? selector : returnValues[selector];
    }

    function afterInitialize(address, PoolKey memory, UQ64x96, int24) external view override returns (bytes4) {
