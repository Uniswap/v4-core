...
describe('TWAMM Hook', () => {
  ...
  async function getDeployedTWAMMCode(poolManagerAddress: string): Promise<string> {
    ...
    await hre.network.provider.send('hardhat_setCode', [
      twammHookAddress,
      await getDeployedTWAMMCode(manager.address),
    ])

    const twamm: TWAMMHook = twammFactory.attach(
      twammHookAddress
    ) as TWAMMHook
  ...
  describe('#beforeInitialize', async () => {
    it('allows initialize of free max range pool', async () => {
      ...
      expect((await twamm.twamm()).lastVirtualOrderTimestamp).to.equal((await ethers.provider.getBlock('latest')).timestamp)
    })
  })
