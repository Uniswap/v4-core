...
contract PoolManagerTest is Test, Deployers, GasSnapshot {
    ...
    event ProtocolFeeUpdated(PoolId indexed id, uint32 protocolFee);
    event Transfer(
    ...
    function test_swap_accruesProtocolFees(uint16 protocolFee1, uint16 protocolFee0) public {
        ...
        uint32 protocolFee = (uint32(protocolFee1) << 16) | (uint32(protocolFee0) & uint32(0xFFFF));
    ...
    function test_setProtocolFee_updatesProtocolFeeForInitializedPool(uint32 protocolFee) public {
        (Pool.Slot0 memory slot0,,,) = manager.pools(key.toId());
        assertEq(slot0.protocolFee, 0);
        feeController.setSwapFeeForPool(key.toId(), protocolFee);

        uint16 fee0 = uint16(protocolFee >> 16);
        uint16 fee1 = uint16(protocolFee % 65536);
    ...
    function test_collectProtocolFees_initializesWithProtocolFeeIfCalled() public {
        uint32 protocolFee = 2500; // 0000000000000000 0000100111000100
