...
contract EndToEnd is PropertiesAsserts, Deployers {
    ...
    uint256 NUMBER_CURRENCIES = 2;
    uint256 NUMBER_LIQUIDITY_ACTORS = 1;
    uint256 NUMBER_DONATION_ACTORS = 1;
    uint256 NUMBER_SWAP_ACTORS = 1;
    ...
    function _setupActorApprovals(IActor c) internal {
        ...
        for (uint256 i = 0; i < NUMBER_CURRENCIES; i++) {
            ...
            for (uint256 approveI = 0; approveI < toApprove.length; approveI++) {
    ...
    function e2e_CreatePool_Coverage(
        uint8 currency1I,
        uint8 currency2I,
        int16 tickSpacing,
        uint256 startPrice,
        ...
        uint256 initialPrice =
            clampBetween(startPrice, 4295128739, 1461446703485210103287273052203988822378723970342 - 1);
    ...
    function e2e_CreatePool(uint8 currency1I, uint8 currency2I, int16 tickSpacing, uint256 startPrice, uint256 fee)
        public
    {
        Currency c1;
        Currency c2;
        ...
        try manager.initialize(k, uint160(initialPrice), ZERO_BYTES) returns (int24 t) {
            ...
            if (bytes4(data) == bytes4(keccak256(bytes("PoolAlreadyInitialized()")))) {
                ...
                assertWithMsg(
                    poolinitialized, "initialize() reverted with PoolAlreadyInitialized() but pool is not initialized"
                );
                return;
            }
            if (bytes4(data) == bytes4(keccak256(bytes("InvalidSqrtPrice(uint160)")))) {
                ...
                assertWithMsg(
                    initialPrice < 4295128739 || initialPrice > 1461446703485210103287273052203988822378723970342 - 1,
                    "initialize() reverted with InvalidSqrtPrice() but initialPrice is within valid range"
                );
                return;
            }
            if (bytes4(data) == bytes4(keccak256(bytes("LPFeeTooLarge(uint24)")))) {
                ...
                assertGt(
                    fee, 1_000_000, "initialize() reverted with LPFeeTooLarge() but initialFee is within valid range"
                );
                return;
            }
            // assertWithMsg(false, "manager.initialize() threw an unknown exception. investigate");
    ...
    function e2e_DonateToLiquidity(uint8 actorIndex, uint256 poolIndex, uint256 amount0, uint256 amount1) public {
    ...
    function e2e_ProvideLiquidityFullRange(uint8 actorIndex, uint256 poolIndex, int256 liquidityDelta) public {
        LiquidityActor actor = _clampLiquidityActor(actorIndex);
        PoolKey memory poolKey = _clampToValidPool(poolIndex);
    ...
    function e2e_SwapOneDirection(uint8 actorIndex, uint256 poolIndex, int256 amountSpecified, bool zeroForOne)
        public
    {
    ...
    function e2e_SwapBidirectional(uint8 actorIndex, uint256 poolIndex, int256 amountSpecified, bool zeroForOne)
        public
    {
        SwapActor actor = _clampSwapActor(actorIndex);
        PoolKey memory poolKey = _clampToValidPool(poolIndex);
