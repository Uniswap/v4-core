...
contract MintActionProps is ActionFuzzBase {
    ...
    function _afterMint() internal {
        ...
        // UNI-SETTLE-8
        assertGte(
            expectedDeltaMint, 0, "After a mint, the sender's currency delta should decrease to reflect increased debt."
        );
        // UNI-SETTLE-9
        assertEq(
            uint256(expectedDeltaMint),
            _mintAmount,
            "After a mint, the difference between the sender's previous and new currency delta should match the mint amount"
        );
        ...
        assertEq(
            recipientDelta,
            _mintAmount,
            "After a mint, the recipient's ERC6909 balance should increase by the mint amount"
        );
