...
import { SwapPaymentTest } from '../typechain/SwapPaymentTest'
...
import { ReentrancyTester } from '../typechain/ReentrancyTester'
...
describe('Pool', () => {
  ...
  // https://github.com/Uniswap/v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (await ethers.getContractFactory('ReentrancyTester')).deploy()) as ReentrancyTester
  ...
  describe('swap underpayment tests', () => {
    let underpay: SwapPaymentTest
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('SwapPaymentTest')
