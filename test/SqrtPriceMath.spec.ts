...
describe('SqrtPriceMath', () => {
  ...
  describe('#getNextSqrtPriceFromInput', () => {
    ...
    it('input amount of 0.1 currency1', async () => {
    ...
    it('input amount of 0.1 currency0', async () => {
      const sqrtQ = await sqrtPriceMath.getNextSqrtPriceFromInput(
        encodeSqrtPriceX96(1, 1),
        expandTo18Decimals(1),
        expandTo18Decimals(1).div(10),
    ...
    it('can return 1 with enough amountIn and zeroForOne = true', async () => {
      expect(
        await sqrtPriceMath.getNextSqrtPriceFromInput(encodeSqrtPriceX96(1, 1), 1, constants.MaxUint256.div(2), true)
      ).to.eq(1)
    })
  ...
  describe('#getNextSqrtPriceFromOutput', () => {
    ...
    it('fails if output amount is exactly the virtual reserves of currency0', async () => {
    ...
    it('fails if output amount is greater than virtual reserves of currency0', async () => {
      const price = '20282409603651670423947251286016'
      const liquidity = 1024
      const amountOut = 5
      await expect(sqrtPriceMath.getNextSqrtPriceFromOutput(price, liquidity, amountOut, false)).to.be.reverted
    ...
    it('fails if output amount is greater than virtual reserves of currency1', async () => {
      const price = '20282409603651670423947251286016'
      const liquidity = 1024
    ...
    it('fails if output amount is exactly the virtual reserves of currency1', async () => {
      const price = '20282409603651670423947251286016'
      const liquidity = 1024
    ...
    it('succeeds if output amount is just less than the virtual reserves of currency1', async () => {
      const price = '20282409603651670423947251286016'
      const liquidity = 1024
    ...
    it('output amount of 0.1 currency1', async () => {
    ...
    it('reverts if amountOut is impossible in zero for one direction', async () => {
      await expect(sqrtPriceMath.getNextSqrtPriceFromOutput(encodeSqrtPriceX96(1, 1), 1, constants.MaxUint256, true)).to
        .be.reverted
    })

    it('reverts if amountOut is impossible in one for zero direction', async () => {
      await expect(sqrtPriceMath.getNextSqrtPriceFromOutput(encodeSqrtPriceX96(1, 1), 1, constants.MaxUint256, false))
        .to.be.reverted
    })
  ...
  describe('#getAmount0Delta', () => {
    it('returns 0 if liquidity is 0', async () => {
      const amount0 = await sqrtPriceMath.getAmount0Delta(encodeSqrtPriceX96(1, 1), encodeSqrtPriceX96(2, 1), 0, true)

      expect(amount0).to.eq(0)
    })
    it('returns 0 if prices are equal', async () => {
      const amount0 = await sqrtPriceMath.getAmount0Delta(encodeSqrtPriceX96(1, 1), encodeSqrtPriceX96(1, 1), 0, true)
    ...
    it('works for prices that overflow', async () => {
      const amount0Up = await sqrtPriceMath.getAmount0Delta(
        encodeSqrtPriceX96(BigNumber.from(2).pow(90), 1),
        encodeSqrtPriceX96(BigNumber.from(2).pow(96), 1),
        expandTo18Decimals(1),
        true
      )
    ...
    it(`[ @skip-on-coverage ] gas cost for amount0 where roundUp = true`, async () => {
      await snapshotGasCost(
        sqrtPriceMath.getGasCostOfGetAmount0Delta(
          encodeSqrtPriceX96(100, 121),
          encodeSqrtPriceX96(1, 1),
          expandTo18Decimals(1),
          true
        )
  ...
  describe('#getAmount1Delta', () => {
    it('returns 0 if liquidity is 0', async () => {
      const amount1 = await sqrtPriceMath.getAmount1Delta(encodeSqrtPriceX96(1, 1), encodeSqrtPriceX96(2, 1), 0, true)

      expect(amount1).to.eq(0)
    })
    it('returns 0 if prices are equal', async () => {
      const amount1 = await sqrtPriceMath.getAmount0Delta(encodeSqrtPriceX96(1, 1), encodeSqrtPriceX96(1, 1), 0, true)
    ...
    it(`[ @skip-on-coverage ] gas cost for amount0 where roundUp = false`, async () => {
      await snapshotGasCost(
        sqrtPriceMath.getGasCostOfGetAmount0Delta(
          encodeSqrtPriceX96(100, 121),
          encodeSqrtPriceX96(1, 1),
